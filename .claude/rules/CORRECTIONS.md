...
| `SendUserFile` の `files` は配列必須。生パス文字列をそのまま渡すと `__unparsedToolInput` の JSON パースに失敗し `InputValidationError` になる                                                                                                                                        | `SendUserFile`                              |
| switch 表に無い bun 専用サブコマンド（`pm`, `outdated` 等）は `na <subcmd>` へ書き換わり、na はカレントのロックファイルから別マネージャを解決するため、bun 限定の診断コマンドが別マネージャのエラーで返る                                                                            | `hooks/pre-bash/package_manager_rewrite.py` |
| client_identifier_gate.py が実行属性を失うと PreToolUse:Bash が exit 126 で非ブロッキング失敗し、識別子ブロックが効かないまま毎 Bash コマンドが通過する。会話にも `git diff` にも出ず、気付く経路は transcript の `hook_non_blocking_error` のみ                                     | `hooks/pre-bash/client_identifier_gate.py`  |
