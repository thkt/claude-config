...
| `uvx <tool>` はサンドボックス内で `~/.cache/uv` 配下の `.git` への書込が `Operation not permitted` になり失敗する。回避は `dangerouslyDisableSandbox: true` を付けて再実行する                                                                                                                                                                                                                         | `rules/core/OPERATION.md`                          |
...
| git の範囲指定は欠けた分を無言で埋める。`git stash push <pathspec>` は pathspec 外の staged 変更も退避して rename で pop を衝突させ、`git log --since=<date>` は欠けた時刻を実行時刻で埋める。結果がもっともらしく出るので、気付く経路は `--name-only` と `--since` の時刻付き結果との照合だけ | `rules/core/OPERATION.md`           |
