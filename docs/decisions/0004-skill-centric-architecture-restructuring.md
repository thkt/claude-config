...
**理由**: ADR はドキュメントではなくアーキテクチャ決定の記録。トップレベルに配置することで重要性を明示。

### 4. コマンドの薄いラッパー化

ADR 0001/0002 パターンに従い、全コマンドを以下の構造に統一：
...
**削減効果**: 多くのコマンドが 200〜300 行 → 80〜150 行に圧縮
...
ADR 0001/0002/0003 から強制ルールを抽出：

| 生成ルール                | 抽出元         | 配置先               |
| ------------------------- | -------------- | -------------------- |
| `MODULARIZATION_RULES.md` | ADR 0001, 0002 | `rules/workflows/`   |
...
- **保守性**: 共通ロジックが skills/に集約され、重複が削減
- **ルール強制**: ADR から抽出したルールにより、パターン違反を防止
...
- [ADR 0002](./0002-fix-modularization-and-tdd-commonization.md) - 3 層アーキテクチャ
