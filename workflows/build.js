  ...
  anchor(
    commitInstruction +
      ...
      `(3) append the fact tail, then open the pull request. ${ghOrMcpRoute(
        ...
        `shell \`&&\` cannot chain into a tool call, so enforce the invariant yourself: from the repository root run \`python3 ${bundled("workflows/build/pr-body.py")} < {tempfile} >> {bodyfile}; echo "renderer-exit=$?"\`. Not whether {bodyfile} reads back: step (1) already wrote the human-facing body into it, so it reads back in full even on a run where pr-body.py exited non-zero and appended nothing. When renderer-exit is anything but 0, do not create the pull request; report committed with an empty pr_url and the error instead. Otherwise call the mcp__github__create_pull_request tool with draft: true${baseBranch ? `, base: ${JSON.stringify(baseBranch)}` : ""}, title: {title}, and body set to the body file's full contents.`,
      )}\n` +
