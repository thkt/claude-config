...
`{...}` is replaced with content at generation. Sections marked `(optional)` are omitted, heading and all, when there is nothing to say. Fixed items (the decided changes, the extent) stay unmarked; tentative items (an unverified premise, an inferred procedure) carry a `(tentative: <action at pickup>)` mark (criteria in SKILL.md § Confidence marking). There is no Premises section here, so issue-level premises are marked inline too.
