...
Detect and fix LLM-style empty phrasing in the PR body. Use alongside prose-review.md when the body is in English. Fix empty phrasing of the same nature even when the table below does not name it. The Signal column gives detection cues, so extend each one to phrasings of the same shape.
