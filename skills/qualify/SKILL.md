...
| Displayed field enumeration | When an issue adds or changes a displayed domain field, it enumerates that field in the AC or cites an agent-readable source. A missing enumeration's finding names the AC and the plan's T-NNN as the destination | blocker  |
...
Address the questions to the user. The user chooses whether to settle the answers or send the questions to the issue author.
