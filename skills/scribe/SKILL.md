...
2. Settle the globs of the implementation files the rule bears on, and separately settle the scenes it bears on from `find_wiki_rule.py`'s `SCENES` constant. A rule confined to filing or PR practice carries an empty globs array; that names no implementation file, and it does not mean the rule bears on no scene. Judge scenes on their own evidence instead of reading them off the globs result
...
4. Run `python3 ${CLAUDE_SKILL_DIR}/scripts/verify_run.py <worktree> <start-count> <expected-commits> <base> <created>`. `<start-count>` is the 昇格待ち row count before the write, `<expected-commits>` is the element count of `commits`, `<base>` is the `origin/<default branch>` step 1 branched from, `<created>` is how many of `pages` carry `action: create`, and confirm `ok` comes back true. On false, do not go on to step 5
