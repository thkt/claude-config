...
1. Read `docs/wiki/*.md` to grasp the existing pages. A page whose frontmatter carries `kind: structure` describes structure rather than a pattern. Even when the name coincides, do not set `existing: "page"` for it; treat it as `none` and open a new page
...
| Do a structure page's boundaries / contracts / requirements match the current implementation? | The wording that restates it to match the current code. Do not drop it |
