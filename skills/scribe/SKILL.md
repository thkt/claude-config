...
2. Write what Phase 3-5 settled, inside the worktree. Pages select the skeleton in ${CLAUDE_SKILL_DIR}/templates/page.md by kind, an item without `kind: structure` follows the 共通項 skeleton and one with `kind: structure` follows the structure skeleton, candidate lines go to `_candidates.md` in Phase 3 step 7's form, and the reference and 由来 repairs use the relink targets Phase 4-5 settled
