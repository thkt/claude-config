...
4. Run `python3 ${CLAUDE_SKILL_DIR}/scripts/verify_run.py <worktree> <waiting-at-base> <commits-count> <base> <created>`. `<waiting-at-base>` is the 昇格待ち row count read from `git -C <worktree> show <base>:docs/wiki/_candidates.md`, `<commits-count>` is the element count of `commits` step 3 settled, `<base>` is the `origin/<default branch>` step 1 branched from, `<created>` is how many of `pages` carry `action: create`, and confirm `ok` comes back true. On false, do not go on to step 5
