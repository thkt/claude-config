...
12. A seam unit's files include at least one non-test file: the one that makes the connection. Without one the wiring is already a preceding unit's work, so place no seam unit and put the cross-boundary assertion among the tests of the unit that makes the wiring. Should that unit then exceed the caps, it carries two outcomes and splits per step 10
13. Once the units are settled, run `python3 ${CLAUDE_SKILL_DIR}/../scribe/scripts/find_wiki_rule.py docs/wiki <slug> <the units[].files> --scene plan` and diff it against what Phase 2 read. Every page under `matched` is either cited or written off in the prose with the reason it does not bear on this plan. A page under `related` only shares a word, so state why it bears when citing one. Every page under `scenes` is cited
