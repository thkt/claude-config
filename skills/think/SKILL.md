...
12. A seam unit's files include at least one non-test file: the one that makes the connection. When a preceding unit already carries the non-test file, the unit carrying the non-test file becomes the seam, and the cross-boundary assertion goes into the tests of the unit that makes the wiring. Should that unit then exceed the caps, it carries two outcomes and splits per step 10. When no unit carries a non-test file, re-cut the units so one does
