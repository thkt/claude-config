...
1. Assign sequential ids in U-001 / T-001 format, with T-NNN unique across the whole plan. Where the target repo's tests carry prefixed ids, follow that convention and number as T-SK077, continuing from that prefix's repo-wide max. A plan that stays bare while the repo is prefixed leaves the rename to implementation time
