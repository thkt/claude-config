...
`{...}` is replaced with content at generation. The draft consists of exactly these 2 sections; keep the headings and bullet shapes intact. The build workflow maps the Plan section into build.js's EXTRACT_SCHEMA via LLM extraction. A deterministic cross-check of the U-NNN / T-NNN id sets stops omissions and fabrications. Break the skeleton and that extraction breaks with it. No hidden machine block.
...
- conventions: {shared conventions later units must keep (composed components, where formatting is written, how state is passed)}

### Rules

{Rules that bear on more than one unit, each with its source and a verbatim quote. Omit the whole subsection when there are none}
