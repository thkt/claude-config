...
The globals actually passed into the script body are exactly seven: `agent`, `workflow`, `parallel`, `pipeline`, `phase`, `log`, and `args` (the parameter list of `vm.compileFunction` in `workflows/_lib/run-workflow.js`). `crypto`, `fetch`, `process`, and `Buffer` do not exist, and referencing any of them throws `ReferenceError`; the message "crypto is not defined" is the literal text observed in production (issue #317's Premises). `Date.now()`, `Math.random()`, and argument-less `new Date()` are also replaced with an Error citing resume as the reason, while `new Date()` called with arguments and `Math.floor` are unaffected. Code generation from strings (`eval`, `new Function`) is disabled as well.
