...
workflow は他の workflow を 1 階層だけ入れ子にできる。ルーティング表を写経せずに再利用するための構成である。`audit` と `polish` の meta は build から入れ子で呼ばれると書いているが、`build.js` はどちらも呼ばない。重い保証は draft PR に対して人が起動する。
