  ...
  const translated = await agent(
    anchor(
      `\`$HOME/.claude/settings.json\` から \`language\` を読む (未設定なら english)。` +
        ...
        `- 入力にある主張を過不足なく訳す。\n` +
        `- 1 文は 60 字以内。超える文は主語と述語の切れ目で割る。文を割っても主張の数は変わらないので、粒度を落とさずに割る。逐語で保持する要素を含む文は、この上限の対象外。\n` +
        `- 1 文に置く主張は 1 つ。根拠、実証した結果、別件の指摘は、それぞれ別の文にする。\n` +
        `- em-dash (—) や読点で節を連ねない。接続詞を置くか文を割る。\n` +
        `- \`kind\` が \`finding\` の要素は、主張と根拠の指し先だけを残して 4 文以内にする。\n` +
        `- \`kind\` が \`assumption\` または \`anomaly\` の要素は、主張を 1 つも落とさずに訳す。人間が veto を判断する材料になる。\n` +
