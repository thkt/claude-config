...
`{...}` は生成時に内容へ置き換える。`(任意)` のセクションは書くことがなければ見出しごと省略する。確定の項目 (再現手順、expected と actual) は無印にする。仮の項目 (原因の仮説、推論した修正方針) には `(tentative: <着手時のアクション>)` を付ける。判定の基準は SKILL.md § 確信度マーキングにある。
