...
`{...}` は生成時に内容へ置き換える。`(任意)` のセクションは書くことがなければ見出しごと省略する。確定の項目 (ask, AC, 決定済みの制約) は無印にする。仮の項目 (AI が推論した HOW、未決定の判断) には `(tentative: <着手時のアクション>)` を付ける。判定の基準は SKILL.md § 確信度マーキングにある。
...
- {方針を選んだ理由 (任意): `ネイティブ <dialog> を採用。フォーカス管理が標準で付くため (tentative: 着手時に判断)`}

## Approach (任意)

- {仮の実装方針。確定要件ではなく出発点: `既存構成に合わせて OrderService 配下に配置 (tentative: 着手時に判断)`}
