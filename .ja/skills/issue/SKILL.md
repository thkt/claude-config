...
3. plan 下書きがあれば、本文と Plan の重複を「重複の照合」の手順で突き合わせる。前項までの編集を終えた本文を対象にし、Phase 2 の最後に置く。会話に `/think` の plan 下書きがあればそれを使い、無ければ `.claude/workspace/planning/` の該当ファイルを読む。比べるのは本文の散文と plan 下書きであり、plan 下書きの写し同士ではない。plan 下書きが無ければ、この照合を省く

### 重複の照合

対象は本文と `## Plan` で同じ知識が重なる箇所すべてで、実装方針に限らない。同じ知識かどうかは、片方を直すともう片方も直る関係かで判定し、独立に変わりうるものは両方に残す。重複した本文側は、その見出しが何をする変更かを述べる 1 行を残したうえで `## Plan` への参照に置き換える。参照の向きは本文から `## Plan` へ固定する。plan は `/think` が本文より先に独立したファイルへ書き出すので、Plan 側から本文の節を指す参照はその時点で書けない。食い違うときは plan を正として本文を直し、却下理由と根拠の file:line、痛みの記述は本文に残す。

| 本文の節              | Plan 側の対応    | 扱い                                              |
| --------------------- | ---------------- | ------------------------------------------------- |
| Approach              | unit の contract | 参照へ置き換える                                  |
| Testing Decisions     | T-NNN            | 参照へ置き換える                                  |
| Premises              | 前提             | 参照へ置き換える                                  |
| Scope の In scope     | files            | 参照へ置き換える                                  |
| What & Why            | 無し             | 本文に残す。Plan に対応物が無い                   |
| Acceptance Criteria   | Outcome          | 本文に残す。人間のマージ判断用で build に届かない |
| Scope の Out of scope | 無し             | 本文に残す。build が読まない                      |
