...
3. plan 下書きがあれば、前項までの編集を終えた本文を「重複の照合」の手順で照合する。会話に `/think` の plan 下書きがあればそれを使い、無ければ `.claude/workspace/planning/` の該当ファイルを読む。plan 下書きが無ければ、この照合を省く

### 重複の照合

照合するのは本文の散文と plan 下書きの中身で、plan 下書きの写し同士ではない。対象は本文と `## Plan` で同じ知識が重なる箇所すべてで、実装方針に限らない。同じ知識かどうかは、片方を直すともう片方も直る関係かで判定し、独立に変わりうるものは両方に残す。重複した本文側は `## Plan` への参照に置き換える。置き換えたあとも、その見出しが何をする変更かを述べる 1 行、却下理由と根拠の file:line、痛みの記述は本文に残す。参照の向きは本文から `## Plan` へ固定する。plan は `/think` が本文より先に独立したファイルへ書き出すので、Plan 側から本文の節を指す参照はその時点で書けない。食い違うときは plan を正として本文を直す。
