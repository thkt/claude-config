...
オプション、出力形式、終了コードは `codegraph <subcommand> --help` で調べる。インストール済みの版に対する正はこの help 出力。
...
| バイナリ | bun global (`~/.config/bun/bin/codegraph`)。インストールや更新で EPERM が出たら `npm_config_cache=$TMPDIR/cg` を前置して回避する   |
