...
build は本文の U-NNN/T-NNN の id 集合と抽出結果を厳密比較する。qualify は抽出を行わないので、代わりに本文側の id が一意で連番になっているかを見る。重複と欠番は blocker。
...
issue の書式が `/issue` の出力形式に沿うかを見る。ここでの違反は build を止めないので advice として扱う。ただし AC が検証不能な場合だけは blocker にする。実装した結果が正しいかを誰も判定できず、build の conformance も照合先を失うため。
...
verdict は上から順に判定し、最初に該当したものを採る。
...
出力は会話に返す。構成は verdict 1 行、blocker 一覧、advice 一覧、質問ドラフトの順。blocker と advice が 0 件の節は「なし」と書く。
