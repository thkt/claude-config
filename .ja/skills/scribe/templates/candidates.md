...
Phase 1 で `docs/wiki/_candidates.md` が無いとき、/scribe が下の内容をそのまま書き出す。置換は無い。3 つの節は空でも消さない。`triage.py` が「昇格待ち」と「単発」を読み、Phase 3 手順 7 が両方へ書き分け、Phase 4 手順 5 が「棄却」へ移すので、節が無いと行の置き場が無くなる。
