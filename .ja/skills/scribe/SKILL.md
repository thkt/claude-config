...
2. その決まりごとが効く実装ファイルの glob と、`find_wiki_rule.py` の `SCENES` 定数から選ぶ scenes を、それぞれ決める。起票や PR の運用に閉じる決まりごとは glob を空配列にするが、これは実装ファイルが無いことを示すだけで、どの scene にも属さないことは意味しない。scenes は glob の結果から読み取らず、それ自体の根拠で判定する
