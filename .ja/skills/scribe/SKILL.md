...
4. `python3 ${CLAUDE_SKILL_DIR}/scripts/verify_run.py <worktree> <start-count> <expected-commits> <base>` を実行する。`<start-count>` は書き込み前の昇格待ち行数、`<expected-commits>` は `commits` の要素数、`<base>` は手順 1 で分岐した `origin/<デフォルトブランチ>` とし、`ok` が true であることを確認する。false なら手順 5 へ進まない
