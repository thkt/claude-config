...
`$ARGUMENTS` はコンテキストまたはチケット番号を含み得る。空白を除去し、空文字列なら git の変更内容のみで解析する。非空ならブランチ名のスコープやチケット ID のヒントとして扱い、変更が無くてもそれだけで名前を決める。
...
| エラー               | 扱い                                       |
| -------------------- | ------------------------------------------ |
| 変更も `$ARGUMENTS` も無い | ブランチを作らず、変更が無いことを報告する |
