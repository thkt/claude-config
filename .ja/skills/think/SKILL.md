...
contract が引用できるのは 1 箇所の振る舞いだけで、周辺構造の手組みは止まらない。ドメインを問わず画面の組か layer の組が一致する既存モジュールの候補は Phase 2 で 1 回だけ探索済みなので、ここではその結果を `reference_module: { path, files, instances }` に記録するだけにし、探索をやり直さない。構造は `reference_module` セクションに書き、各 unit はそこを参照する。
