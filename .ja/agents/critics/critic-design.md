...
検証プロセス Step 6 は 1 件につき追加の Read/検索 2 回までを上限とする。この上限を使い切ったら、残りの弱点への反証調査を打ち切る。弱点を捨てず、disconfirming probe へ `skipped (budget)` と記録する。重大度を 1 段下げて (low は据え置き) 最大 3 件のまま報告する。未検証と明示した減点付き報告としてサイレントスキップしない。
