...
ビルド/型エラーを可能な限り小さな diff で解決する。症状を隠さず根本原因を修正し、リファクタやアーキテクチャ変更を伴わずビルドが exit 0 になる状態へ持っていく。
...
Agent 完了時に以下のフィールドを返す。エラーがなく Build clean を報告する場合も有効な結果であり、エラーではない。
