...
T-NNN はファイル単位で一意にする。plan は T-001 から番号を振るが、その plan のテストをファイルへ落とすときは、そのファイルで使われている最大値の次から振り直す。plan ごとに 1 から振り直すと、同じファイルへ次の plan のテストが足された時点で番号が重なり、テスト名から出所の plan を辿れなくなる。番号を変えても build の Verify は plan の言明文を照合するので、照合は外れない。
